...
{    
    ...
    [weak_this{weak_from_this()}](std::shared_ptr<Event> event, short events)
    {
        if (auto shared_this = weak_this.lock(); shared_this != nullptr)
            shared_this->_DoConnect(event->GetSocket(), events);
    });
...
void TcpClient::_DoConnect(int socket, short events)
{
    ...
    if (events & EventOpt::TIMEOUT) {
        m_on_connect(FASTERR_ERROR("connect timeout!"));
        goto ConnectFinal;
    }
    ...
    if (events & EventOpt::WRITEABLE) {
        if (::connect(socket, m_serv_addr.getsockaddr(), m_serv_addr.getsocklen()) != 0) {
            ...
            if (err == ECONNREFUSED) {
                m_on_connect(FASTERR_ERROR("connect refused!"));
                goto ConnectFinal;
            }
        }
    }
...
ErrOpt TcpClient::Send(bbt::core::Buffer& buffer)
{
    Assert(false);
}
